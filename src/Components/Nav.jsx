import React from 'react'
import { Link } from 'react-router-dom'

function Nav() {
  return (
    <div>
    <div className='nav bg-[#131921] h-[90px] '>
        <div className="logo">
            <img className='h-[80px]  w-[100px]' src="data:image/png;base64," alt="" />

        </div>
        <div className="sign flex text-[#fff]">
            <Link to='/ShoppingCard'>
            <div>
            <li>Card    <i class="fa-solid fa-basket-shopping"></i></li>
         
            </div>
       
            </Link>
           
            {/* <img src="https://png.pngtree.com/element_our/20190531/ourmid/pngtree-shopping-cart-convenient-icon-image_1287807.jpg" alt="" /> */}
            <Link to='/Signin'>
            <li>Login</li>
            </Link>
            <Link to='/SignNew'>
            <li>SignUp</li></Link>
       
        </div>

    </div>
    <div className='bg-[#232f3e] pr-[10px] pt-[10px] pb-[10px]'>
            <ul className='flex text-[#fff]'>
                <li className='mr-[10px]'>All</li>
                <li className='mr-[10px]'>Today's Deals</li>
                <li className='mr-[10px]'>customer Services</li>
                <li className='mr-[10px]'>Registory</li>
                <li className='mr-[10px]'>Gift Cards</li>
                <li className='mr-[10px]'>Sell</li>
            </ul>
        </div>
        <div/>
    </div>

  )
}


export default  Nav